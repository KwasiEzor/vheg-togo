@props(['fund','rate'=>null])

@php
$classes = "w-full h-full border-2 border-transparent shadow-md rounded-xl bg-white"
@endphp

<article {{ $attributes(['class'=>$classes]) }}>
    <div class="relative w-full overflow-hidden">
        <img src="{{ $fund->cover_img }}" alt="food for kids" class="w-full h-full max-w-fit rounded-t-xl" />
        <span
        ...
        <div class="flex flex-col items-center space-y-3">
            <div class="relative w-full pt-12">
                <span
                    ...
                    style="left: 50%">{{ $fund->calculateFundRate($rate) }}%</span>
            ...
            <div class="flex items-center justify-between w-full my-4 font-DM-Serif">
                <div class="inline-flex space-x-2">
                    <p class="font-bold text-gray-600">${{ $fund->calculateFundRate($rate) }}</p>
                    <p class="font-normal text-gray-400">Raised now</p>
                </div>
                <div class="inline-flex space-x-2">
                    <p class="font-bold text-gray-600">${{ $fund->amount }}</p>
